...
func generateBase(basePath string) error {
	...
	exists, err := files.FileOrDirExists(filepath.Join(basePath, "excluded"))
	if err != nil && exists {
		excludedResources, err = files.EnumerateAllFiles(filepath.Join(basePath, "excluded"))
		if err != nil {
			return err
		}
	}

	kData, err := baseKustomization(manifestResources, excludedResources)
	if err != nil {
		return err
	}
...
func baseKustomization(manifestResources, excludedResources []string) ([]byte, error) {
...
{{ end }}`
	...
	if err := tmpl.Execute(buf, templateData{manifestResources, excludedResources}); err != nil {
		return []byte{}, err
	}
